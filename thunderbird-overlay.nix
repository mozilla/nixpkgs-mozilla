# This file provide the latest binary versions of Thunderbird published by Mozilla.
# It is a slightly modified version of firefox-overlay.nix
self: super:

let
  # This URL needs to be updated about every 2 years when the subkey is rotated.
  pgpKey = super.fetchurl {
    url = "https://download.cdn.mozilla.net/pub/thunderbird/releases/115.0/KEY";
    sha256 = "2c67dd09f369a7c1d3a9e139455e5c8d40be8cc04441a790a103019900fd194d";
  };
  ...
  thunderbird_versions = with builtins;
    fromJSON (readFile (fetchurl "https://product-details.mozilla.org/1.0/thunderbird_versions.json"));
  ...
  versionInfo = { name, version, release, system ? arch, timestamp ? null, info ? null, ... }: with builtins;
    ...
    if release then
      ...
      # https://download.cdn.mozilla.net/pub/thunderbird/releases/55.0b3/SHA256SUMS
      let
        dir = "https://download.cdn.mozilla.net/pub/thunderbird/releases/${version}";
        # TODO: Update the extension once XZ linux builds leave nightly channel
        file = "${system}/en-US/thunderbird-${version}.tar.bz2";
      ...
      # https://download.cdn.mozilla.net/pub/thunderbird/nightly/latest-comm-central/thunderbird-56.0a1.en-US.linux-x86_64.checksums
      let
        dir =
          if timestamp == null then
            let
              buildhubJSON = with builtins;
                fromJSON (readFile (fetchurl "https://download.cdn.mozilla.net/pub/thunderbird/nightly/latest-comm-central/thunderbird-${version}.en-US.${system}.buildhub.json"));
            in builtins.replaceStrings [ "/${file}" ] [ "" ] buildhubJSON.download.url
          else "https://download.cdn.mozilla.net/pub/thunderbird/nightly/${yearOf timestamp}/${monthOf timestamp}/${timestamp}-comm-central" ;
        file = "thunderbird-${version}.en-US.${system}.tar.xz";
        sha512Of = chksum: file: head (match ".*[\n]([0-9a-f]*) sha512 [0-9]* ${file}[\n].*" (readFile (fetchurl chksum)));
      in rec {
        chksum = "${dir}/thunderbird-${version}.en-US.${system}.checksums";
        chksumSig = null;
        # file content:
        # <hash> sha512 62733881 thunderbird-56.0a1.en-US.linux-x86_64.tar.bz2
        # <hash> sha256 62733881 thunderbird-56.0a1.en-US.linux-x86_64.tar.bz2
        url = "${dir}/${file}";
        sha512 = sha512Of chksum file;
        sig = "${dir}/${file}.asc";
  ...
  verifyFileAuthenticity = { file, sha512, chksum, chksumSig }:
    ...
    super.runCommand "check-thunderbird-signature" {
  ...
  thunderbirdVersion = version:
    let
      info = versionInfo version;
      pkg = ((self.thunderbird-bin-unwrapped.override {
        generated = {
  ...
  thunderbirdVariants = {
    thunderbird-nightly-bin = {
      ...
      channel = "nightly";
      wmClass = "thunderbird-nightly";
      version = thunderbird_versions.LATEST_THUNDERBIRD_NIGHTLY_VERSION;
      release = false;
    };
    thunderbird-beta-bin = {
      name = "Thunderbird Beta";
      channel = "beta";
      wmClass = "thunderbird-beta";
      version = thunderbird_versions.LATEST_THUNDERBIRD_DEVEL_VERSION;
      release = true;
    };
    thunderbird-bin = {
      name = "Thunderbird";
      channel = "release";
      wmClass = "thunderbird";
      version = thunderbird_versions.LATEST_THUNDERBIRD_VERSION;
      release = true;
    };
    thunderbird-esr-bin = {
      ...
      channel = "release";
      wmClass = "thunderbird";
      version = thunderbird_versions.THUNDERBIRD_ESR;
      release = true;
    };
  };
in

{
  lib = super.lib // {
    thunderbirdOverlay = {
      inherit pgpKey thunderbirdVersion versionInfo thunderbird_versions thunderbirdVariants;
    };
  };

  # Set of packages which are automagically updated. Do not rely on these for
  # reproducible builds.
  latest = (super.latest or {}) // (builtins.mapAttrs (n: v: thunderbirdVersion v) thunderbirdVariants);
